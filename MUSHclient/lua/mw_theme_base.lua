...
function Draw3DRect (win, left, top, right, bottom, sunken, gradient)
   if right > 0 then
      right = right + 1
   end
   if bottom > 0 then
   ...
   if gradient then
      WindowGradient(win, left, top, right, bottom,
                theme.TITLE_GRADIENT_FIRST,
                theme.TITLE_GRADIENT_SECOND,
                gradient)
   else
...
function DrawResizeTag (win, x1, y1, size)
    ...
    while (x1+m+2 <= x2-1 and y1+n+1 <= y2-2) do
        WindowLine(win, x1+m+1, y2-2, x2-1, y1+n, theme.THREE_D_OUTERHIGHLIGHT, 0, 1)
...
function DrawTitleBar(win, title, text_alignment, full_width)
   ...
   if full_width then
      left = 0
      right = WindowInfo(win, 3)-1
   else
      left = math.max(0, text_left - theme.TITLE_PADDING)
      right = math.min(text_left + title_width + theme.TITLE_PADDING, WindowInfo(win, 3))
   end
