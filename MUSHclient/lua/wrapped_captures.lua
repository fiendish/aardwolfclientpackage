...
function command(command_to_send, capture_start_tag, capture_end_tag, regexp, silent, omit, no_prompt, call_with_result)
   ...
   if (not no_prompt) and (prompt_mode == "YES") then
      if compact_mode == "NO" then
   ...
   AddTriggerEx(
      "tag_captures_module___terminate_"..i,
      "{{"..GetPluginID().." TAG TERMINATE "..i,
      "Capture.___terminate('"..i.."');StopEvaluatingTriggers(true)",
      trigger_flag.OmitFromLog + trigger_flag.OmitFromOutput + trigger_flag.Temporary + trigger_flag.Enabled + trigger_flag.OneShot,
      -1, 0, "", "", sendto.script, ___storage[i]["sequence_low"]
   )
   SendNoEcho("echo {{"..GetPluginID().." TAG TERMINATE "..i)

   if no_prompt and (prompt_mode == "YES") then
      Send_GMCP_Packet("config prompt YES")
   end

   if (compact_mode == "NO") and not ((not no_prompt) and (prompt_mode == "YES")) then
      Send_GMCP_Packet("config compact NO")
   end
