...
function tagged_output(command_to_send, -- The command to send to the game.
...
--    end_line is the text of the line with the end tag
    ...
    command(command_to_send, capture_start_tag, capture_end_tag,
            tags_are_regexp, no_command_echo, omit_response_from_output,
            no_prompt_after, callback_function, send_via_execute, manual_tags,
            timeout_callback, timeout_duration)
end

function untagged_output(command_to_send, -- The command to send to the game.
no_command_echo, -- A boolean indicating whether the command should be echoed to the screen (true means omit, false/nil means show).
                         omit_response_from_output, -- A boolean indicating whether the response to the command should be shown in the output window (true means omit, false/nil means show).
                         no_prompt_after, -- A boolean indicating whether the prompt should be displayed after the command is done (true means no prompt, false/nil means yes prompt).
                         callback_function, -- A function to be called when the captured text has been obtained which should take three arguments: captured_styles, start_line, and end_line.
...
function ___capture_end(name, line, wildcards, styles)
    ...
    ___storage[i]["callback"](___storage[i]["captured_lines"],
                              ___storage[i]["start_line"], line)
end
...
function ___create_capture(i, start_line)
    ...
    AddTriggerEx("tag_captures_module___body_" .. i, ".*",
                 "StopEvaluatingTriggers(true)",
                 trigger_flag.RegularExpression +
                     (omit_response_from_output and trigger_flag.OmitFromLog or
                         0) +
                     (omit_response_from_output and trigger_flag.OmitFromOutput or
                         0) + trigger_flag.Temporary + trigger_flag.Enabled, -1,
                 0, "", "Capture.___capture_body", sendto.script,
                 ___storage[i]["sequence_high"])
    AddTriggerEx("tag_captures_module___end_" .. i, end_tag,
                 "DeleteTrigger('tag_captures_module___body_" .. i ..
                     "');StopEvaluatingTriggers(true)",
                 (tags_are_regexp and trigger_flag.RegularExpression or 0) +
                     trigger_flag.OmitFromLog + trigger_flag.OmitFromOutput +
                     trigger_flag.Temporary + trigger_flag.OneShot +
                     trigger_flag.Enabled, -1, 0, "", "Capture.___capture_end",
                 sendto.script, ___storage[i]["sequence_low"])
end
...
function ___terminate(i)
    ...
    for k, v in pairs(___storage) do return end
    ___sequence = 1
end

___sequence = 1

function command(command_to_send, capture_start_tag, capture_end_tag,
                 tags_are_regexp, no_command_echo, omit_response_from_output,
                 no_prompt_after, callback_function, send_via_execute,
                 manual_tags, timeout_callback, timeout_duration)
    ...
    if manual_tags and my_level >= 202 then
        ...
        AddTriggerEx("tag_captures_module___immortal_start_" .. i,
                     "You echo " .. capture_start_tag .. " to " .. my_name ..
                         ".", "StopEvaluatingTriggers(true)",
                     trigger_flag.OmitFromLog + trigger_flag.OmitFromOutput +
                         trigger_flag.Temporary + trigger_flag.Enabled +
                         trigger_flag.OneShot, -1, 0, "", "", sendto.script,
                     ___sequence)
        AddTriggerEx("tag_captures_module___immortal_end_" .. i, "You echo " ..
                         capture_end_tag .. " to " .. my_name .. ".",
                     "StopEvaluatingTriggers(true)",
                     trigger_flag.OmitFromLog + trigger_flag.OmitFromOutput +
                         trigger_flag.Temporary + trigger_flag.Enabled +
                         trigger_flag.OneShot, -1, 0, "", "", sendto.script,
    ...
    -- start trigger
    AddTriggerEx("tag_captures_module___start_" .. i,
                 echo_prefix .. capture_start_tag,
                 "Capture.___create_capture('" .. i ..
                     "', '%0');StopEvaluatingTriggers(true)",
                 trigger_flag.OmitFromLog + trigger_flag.OmitFromOutput +
                     trigger_flag.Temporary + trigger_flag.Enabled +
    ...
    if prompt_mode == "YES" then Send_GMCP_Packet("config prompt NO") end

    if manual_tags then SendNoEcho(echo_command .. capture_start_tag) end
    ...
    if manual_tags then SendNoEcho(echo_command .. capture_end_tag) end

    if compact_mode == "NO" then Send_GMCP_Packet("config compact NO") end
    if prompt_mode == "YES" then Send_GMCP_Packet("config prompt YES") end

    if not no_prompt_after then SendNoEcho("") end

    TelnetOptionOn(TELOPT_PAGING)
    DoAfterSpecial((type(timeout_duration) == "number" and timeout_duration or
                       20), "Capture.___terminate('" .. i .. "')", sendto.script)
end
